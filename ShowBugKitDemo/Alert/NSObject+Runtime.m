...
+ (NSArray *)objectsWithArray:(NSArray *)array
{
    ...
    // 获取属性列表数组
    NSArray *propertyList = [self propertysList];
...
+ (NSArray *)propertysList
{
    ...
    // ---保存属性数组对象---
    objc_setAssociatedObject(self, propertyListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    return objc_getAssociatedObject(self, propertyListKey);
}

#pragma mark - 获取本类所有 ‘方法‘ 的数组
const void *methodListKey = "methodListKey";
+ (NSArray *)methodList
{
    // 1. 使用运行时动态添加属性
    NSArray *methodsList = objc_getAssociatedObject(self, methodListKey);
    ...
    // 5. 保存方法的数组对象
    objc_setAssociatedObject(self, methodListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
...
/** 获取当前类的所有成员变量 */
const char *ivarListKey = "ivarListKey";
+ (NSArray *)ivarList
{
    
    // 1. 查询根据key 保存的成员变量数组
    NSArray *ivarList = objc_getAssociatedObject(self, ivarListKey);
    ...
    objc_setAssociatedObject(self, ivarListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
...
const char *protocolListKey = "protocolListKey";

+ (NSArray *)protocolList {
    NSArray *protocolList = objc_getAssociatedObject(self, protocolListKey);
    if (protocolList != nil)
    {
        return protocolList;
    }
    ...
    free(protocolLists);
    // 将保存 协议的数组动态添加到 关联对象
    objc_setAssociatedObject(self, protocolListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    return objc_getAssociatedObject(self, protocolListKey);
}
